        ...
        if form.is_valid():
            ...
            if flag == 1:
                ...
                for i in range(1,10):
                    ...
                    if not exist:
                        e = Extend(extend_code=code,extend_user=new_user)
                        e.save()
                        break
...
@login_required
def userinfo_m(request,objectid=None):
    url = None
    user = auth.get_user(request)
    flag = 1
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render_to_response("userinfo_m.html", {'form': form, 'flag': flag},
                                      context_instance=RequestContext(request))
        return HttpResponseRedirect(reverse('userinfo_m'))
    else:
        try:
...
def about_us_m(request):
    p = About_us.objects.filter(name=u"上海辞达金融信息服务有限公司")
    mediareports = MediaReports.objects.filter(status=1)

    return render_to_response('about_m.html',{'mediareports':mediareports,"description":p[0].description}, context_instance=RequestContext(request))

def guide_m(request):
    p = About_us.objects.all()
    p1 = RegistrationAgreement.objects.filter(name="mianzetiaokuan")

    return render_to_response('guide_m.html',{"p1":p1[0].agreement,"description":p[0].about_zhongtou}, context_instance=RequestContext(request))
...
@login_required
def investor_m(request):
    return render_to_response('investor_m.html',{}, context_instance=RequestContext(request))
...
def project_m(request,objectid):

    return render_to_response('project_m.html',{"objectid":[int(objectid),14]}, context_instance=RequestContext(request))
